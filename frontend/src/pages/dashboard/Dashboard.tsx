...
import DeleteConfirmModal from "../User/DeleteConfirmModal";
import type { User, StatData } from "../../types/dashboard";
import {
  Users,
  Briefcase,
  Search,
...
const Dashboard: React.FC = () => {
  const [interns, setInterns] = useState<User[]>([
    {
      ...
      end_date: "2026-06-01",
      social_media: { linkedIn: "https://linkedin.com/in/aman", github: "https://github.com/aman", twitter: "https://twitter.com/aman" },
    ...
    },
    {
      ...
      end_date: null,
      social_media: { linkedIn: "https://linkedin.com/in/sanya", github: "https://github.com/sanya" },
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    },
    {
  ...
  const stats: StatData[] = [
    { label: "Total Interns", value: "1,240", icon: <Users size={22} className="text-blue-600" />, trend: { text: "24 Joined this week", type: "neutral" } },
    { label: "Training Completion", value: "88%", icon: <GraduationCap size={22} className="text-purple-600" />, trend: { text: "Above target", type: "success", icon: <CheckCircle size={14} /> } },
    { label: "Open Positions", value: "15", icon: <Briefcase size={22} className="text-amber-600" />, trend: { text: "5 Urgent", type: "warning", icon: <AlertCircle size={14} /> } },
  ];

  const handleToggleVisibility = (id: string) => {
    setInterns((prev) =>
      prev.map((user) => (user.id === id ? { ...user, is_visible: !user.is_visible } : user))
    );
    ...
    <div className="w-full flex flex-col h-full overflow-hidden bg-[#F8FAFC]">
      ...
      <header className="h-20 bg-white flex items-center justify-between px-8 border-b border-slate-200/60 sticky top-0 z-10 shrink-0">
        <div className="flex flex-col">
          <h2 className="text-[#102359] text-xl font-bold leading-tight">Dashboard Overview</h2>
        ...
        <div className="flex items-center gap-6">
          <div className="hidden md:flex items-center h-10 w-80 bg-white rounded-xl border border-slate-200 px-3 focus-within:border-blue-400 transition-all shadow-sm">
            ...
            <input className="w-full h-full border-none bg-transparent text-sm text-slate-700 focus:ring-0 ml-2 outline-none" placeholder="Search interns, jobs..." type="text" />
          </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="bg-[#081E67] p-6 rounded-2xl shadow-xl text-white relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  <h3 className="text-lg font-bold mb-1">Latest Training Program</h3>
                  <p className="text-slate-300 text-xs mb-6">AI & ML Spring Cohort</p>
